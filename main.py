...
from exporters.export import (
    ...
    PostsJSONExporter,
    RootAnalysisExporter,
    RootAnalysisCSVExporter,
)
...
def save_checkpoint_with_posts(results: dict, root_target: str):
    try:
        ...
        checkpoint_data = {
            ...
            "root_analysis": results.get("root_analysis", {}),
...
async def analyze_root_profile(
    ...
    """Analyze the root profile with professional analysis"""
    debug_logger.info(f"Starting professional analysis for root profile: {root_target}")
    
    try:
        analysis = await psychology_analyzer.analyze_root_profile(root_data, root_posts)
        if analysis:
            logger.info(f"Professional analysis completed for {root_target}")
            return analysis
        else:
            logger.warning(f"No analysis generated for {root_target}")
            return None
    except Exception as e:
        logger.error(f"Error analyzing root profile: {e}", exc_info=True)
        return None
...
async def main():
    ...
    # --- Root Profile Professional Analysis ---
    root_analysis = None
    try:
        logger.info("-" * 60)
        logger.info("Running professional analysis on root profile")

        psychology_analyzer_root = PsychologyAnalyzer()

        if psychology_analyzer_root.is_available():
            logger.info("Psychology analyzer: ENABLED for root analysis")

            root_analysis = await analyze_root_profile(
                psychology_analyzer=psychology_analyzer_root,
                root_data=results.get("root", {}),
                root_posts=root_posts,
                root_target=root_target
            )

            if root_analysis:
                results["root_analysis"] = root_analysis
                logger.info("Root profile professional analysis completed")
                
                root_json_exporter = RootAnalysisExporter()
                root_json_exporter.export(results)
                
                root_csv_exporter = RootAnalysisCSVExporter()
                root_csv_exporter.export(results)
            else:
                logger.warning("Root profile analysis failed")
        else:
            logger.info("Psychology analyzer: DISABLED - skipping root analysis")
    except Exception as e:
        logger.error(f"Error in root profile analysis: {e}", exc_info=True)
    ...
    if results.get("root_analysis"):
        logger.info(f"    - output/root_analysis.json")
        logger.info(f"    - output/root_analysis.csv")
    logger.info(f"    - checkpoints/{root_target}_checkpoint.json")
    logger.info("=" * 60)
    
    # Show root analysis summary if available
    if results.get("root_analysis"):
        root_analysis_data = results["root_analysis"]
        logger.info("-" * 60)
        logger.info("ROOT PROFILE ANALYSIS SUMMARY:")
        logger.info(f"  Finalidad de la cuenta: {root_analysis_data.get('finalidad_cuenta', 'N/A')}")
        logger.info(f"  Publico objetivo: {root_analysis_data.get('publico_objetivo', 'N/A')}")
        logger.info(f"  Engagement estimado: {root_analysis_data.get('engagement_estimado', 'N/A')}")
        logger.info(f"  Tasa de engagement: {root_analysis_data.get('engagement_rate', 0)}%")
        logger.info(f"  Posts analizados: {root_analysis_data.get('total_posts_analyzed', 0)}")
        if root_analysis_data.get('resumen_ejecutivo'):
            logger.info(f"  Resumen: {root_analysis_data.get('resumen_ejecutivo')[:200]}...")
        logger.info("-" * 60)
    ...
    if root_posts_count > 0:
        ...
        for i, post in enumerate(root_posts[:3], 1):
            comments_count_sample = len(post.get('comments_data', []))
            logger.info(f"  {i}. {post.get('url')} (Likes: {post.get('likes')}, Comments: {comments_count_sample})")
